...
describe('fetchIssueables', () => {
    ...
    const defaultParams = {
        type: custom_query_type_1.CustomQueryType.MR,
        scope: 'all',
        state: 'opened',
        wip: '',
        searchIn: '',
    ...
    const setupFetchIssuable = ({ version = '13.9', responses = [] } = {}) => {
        ...
        const createResponseImplementation = (response) => () => ({ response });
    ...
    const fetchIssuablesHelper = (params = {}) => fetchIssuablesRef({ ...baseParams, ...defaultParams, ...params }, 'test');
    const getMockCall = (calls, pattern) => calls.find((call) => call[0].includes(pattern));
    const getProjectUrl = (calls) => getMockCall(calls, '/api/v4/projects')[0];
    const getGroupUrl = (calls) => getMockCall(calls, '/api/v4/groups')[0];
    ...
    describe('handles types', () => {
      ...
      ${custom_query_type_1.CustomQueryType.VULNERABILITY} | ${'all'}            | ${'all'}
      ${custom_query_type_1.CustomQueryType.VULNERABILITY} | ${'dismissed'}      | ${'dismissed'}
      ${custom_query_type_1.CustomQueryType.ISSUE}         | ${'all'}            | ${'all'}
      ${custom_query_type_1.CustomQueryType.ISSUE}         | ${'assigned_to_me'} | ${'assigned_to_me'}
      ${custom_query_type_1.CustomQueryType.ISSUE}         | ${'created_by_me'}  | ${'created_by_me'}
      ${custom_query_type_1.CustomQueryType.MR}            | ${'all'}            | ${'all'}
      ${custom_query_type_1.CustomQueryType.MR}            | ${'assigned_to_me'} | ${'assigned_to_me'}
      ${custom_query_type_1.CustomQueryType.MR}            | ${'created_by_me'}  | ${'created_by_me'}
    `('sets scope based on type: $type', async ({ type, scope, expectation }) => {
      ...
      ${custom_query_type_1.CustomQueryType.EPIC}          | ${getGroupUrl}   | ${'all'} | ${{ include_ancestor_groups: 'true' }} | ${'/groups/TEST_PROJECT/epics'}
      ${custom_query_type_1.CustomQueryType.VULNERABILITY} | ${getProjectUrl} | ${'all'} | ${{ scope: 'all' }}                    | ${'/projects/TEST_PROJECT/vulnerability_findings'}
      ${custom_query_type_1.CustomQueryType.MR}            | ${getProjectUrl} | ${'all'} | ${{ scope: 'all' }}                    | ${'/projects/TEST_PROJECT/merge_requests'}
    ...
    describe('author parameters', () => {
        it('sets no author parameter', async () => {
            setupFetchIssuable();
            await fetchIssuablesHelper({ type: custom_query_type_1.CustomQueryType.ISSUE });
        ...
        it('sets author_username parameter', async () => {
            setupFetchIssuable();
            await fetchIssuablesHelper({ type: custom_query_type_1.CustomQueryType.ISSUE, author: 'testuser' });
            const search = new URLSearchParams(request.mock.calls[1][0]);
        ...
        it('sets author_id parameter if author is found', async () => {
            ...
            await fetchIssuablesHelper({ type: custom_query_type_1.CustomQueryType.MR, author: 'testuser' });
    ...
    it('sets reviewer parameter', async () => {
        setupFetchIssuable();
        await fetchIssuablesHelper({ type: custom_query_type_1.CustomQueryType.MR, reviewer: 'reviewer' });
        const search = new URLSearchParams(request.mock.calls[1][0]);
    ...
    describe('fetchCurrentPipelineProject', () => {
        ...
        let fetchCurrentPipelineProject;
        beforeEach(() => {
...
//# sourceMappingURL=gitlab_service.test.js.map