...
const { toggleResolved, deleteComment, editComment: startEdit, cancelEdit, submitEdit, createComment, cancelFailedComment, retryFailedComment, } = require('./commands/mr_discussion_commands');
...
const { runWithValidProject, runWithValidProjectFile, } = require('./commands/run_with_valid_project');
...
const registerCommands = (context, outputChannel) => {
    const commands = {
        ...
        [USER_COMMANDS.SHOW_MERGE_REQUESTS_ASSIGNED_TO_ME]: runWithValidProject(openers.showMergeRequests),
        ...
        [USER_COMMANDS.OPEN_CURRENT_MERGE_REQUEST]: runWithValidProjectFile(openers.openCurrentMergeRequest),
        ...
        [PROGRAMMATIC_COMMANDS.NO_IMAGE_REVIEW]: () => vscode.window.showInformationMessage("GitLab MR review doesn't support images yet."),
    };
...
const registerCiCompletion = context => {
    const subscription = vscode.languages.registerCompletionItemProvider({ pattern: '**/.gitlab-ci*.{yml,yaml}' }, new CiCompletionProvider(), '$');
...
const activate = context => {
    ...
    vscode.workspace.registerFileSystemProvider(REMOTE_URI_SCHEME, new GitLabRemoteFileSystem(), GitLabRemoteFileSystem.OPTIONS);
