...
const parseQuery = (query, noteableType) => {
    ...
    const queryParams = Object.keys(params).map(k => params[k] ? `${encodeURIComponent(k)}=${encodeURIComponent(params[k])}` : '');
...
async function showSearchInputFor(noteableType) {
    ...
    const query = await getSearchInput('Search in title or description. (Check extension page for search with filters)');
...
async function showProjectAdvancedSearchInput() {
    const repository = await gitExtensionWrapper.getActiveRepositoryOrSelectOne();
    const project = await repository.getProject();
    const query = await getSearchInput('Project Advanced Search. (Check extension page for Advanced Search)');
    ...
    await openers.openUrl(`${instanceUrl}/search?search=${queryString}&project_id=${project.id}&scope=issues`);
}
