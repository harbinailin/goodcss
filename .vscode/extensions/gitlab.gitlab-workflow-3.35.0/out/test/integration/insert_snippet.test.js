...
const { snippetsResponse, snippetWithOneBlobResponse, snippetWithTwoBlobsResponse, } = require('./fixtures/graphql/snippets');
const { getServer } = require('./test_infrastructure/mock_server');
const { GITLAB_URL, REMOTE } = require('./test_infrastructure/constants');
const { createAndOpenFile, closeAndDeleteFile, simulateQuickPickChoice, getRepositoryRoot, updateRepositoryStatus, } = require('./test_infrastructure/helpers');
...
describe('Insert snippet', async () => {
    ...
    it('throws an error when it cannot find GitLab project', async () => {
        ...
        await vscode.commands.executeCommand(USER_COMMANDS.INSERT_SNIPPET);
        assert.match(showErrorMessage.firstCall.args[0], /Project gitlab-org\/nonexistent was not found./);
    });
});
