...
const createJsonEndpoint = (path, response) => rest.get(`${API_URL_PREFIX}${path}`, (req, res, ctx) => {
    return res(ctx.status(200), ctx.json(response));
});
const sortRequestQuery = search => search
    .slice(search.indexOf('?') + 1)
    .split('&')
    .sort()
    .join('&');
const createQueryJsonEndpoint = (path, queryResponseMap) => rest.get(`${API_URL_PREFIX}${path}`, (req, res, ctx) => {
...
const createTextEndpoint = (path, response) => rest.get(`${API_URL_PREFIX}${path}`, (req, res, ctx) => {
    return res(ctx.status(200), ctx.text(response));
});
const createQueryTextEndpoint = (path, queryResponseMap) => rest.get(`${API_URL_PREFIX}${path}`, (req, res, ctx) => {
...
const createPostEndpoint = (path, response) => rest.post(`${API_URL_PREFIX}${path}`, (req, res, ctx) => {
...
const getServer = (handlers = []) => {
    const server = setupServer(graphql.query('GetProject', (req, res, ctx) => {
        if (req.variables.projectPath === 'gitlab-org/gitlab')
            return res(ctx.data(projectResponse));
        return res(ctx.data({ project: null }));
    }), createJsonEndpoint('/version', versionResponse), ...handlers, notFoundByDefault);
...
//# sourceMappingURL=mock_server.js.map