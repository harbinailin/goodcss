...
describe('GitLab tree view', () => {
    ...
    async function openCategory(label) {
        ...
        assert(chosenCategory, `Can't open category ${label} because it's not present in ${categories.map(c => getTreeItem(c).label)}`);
    ...
    it('shows project merge requests assigned to me', async () => {
        ...
        assert.strictEqual(mrItem.iconPath.toString(true), `${GITLAB_URL}/uploads/-/system/user/avatar/2398164/avatar.png`);
    });
