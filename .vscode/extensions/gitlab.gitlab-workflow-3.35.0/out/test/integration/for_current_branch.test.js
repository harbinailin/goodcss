...
describe('GitLab tree view for current branch', () => {
    ...
    const pipelinesForMrEndpoint = createJsonEndpoint('/projects/278964/merge_requests/33824/pipelines', pipelinesResponseWithFixedDate);
    ...
    it('shows detached pipeline and mr for the current branch', async () => {
        ...
        assert.deepStrictEqual(forCurrentBranch.map(i => dataProvider.getTreeItem(i).label), [
            'Pipeline #47',
    ...
    it('handles error for MR API request', async () => {
        ...
        await refresher.refresh();
        const forCurrentBranch = await dataProvider.getChildren();
        assert.deepStrictEqual(forCurrentBranch.map(i => dataProvider.getTreeItem(i).label), ['Pipeline #47', 'No merge request found', 'No closing issue found']);
    });
