...
const note1TextSnippet = 'I think we should still show some message to the user when they try to run';
const note2 = {
...
const noteOnDiff = {
    ...
    bodyHtml: '<p data-sourcepos="1:1-1:210" dir="auto">This is the core improvement. <code>NoteBody</code> sends the <code>note.body</code> to our <code>/api/v4/markdown</code> endpoint to render HTML. For labels, we can easily render the HTML ourselves, saving all the API requests and complexity.</p>',
    position: {
        diffRefs: {
...
const noteOnDiffTextSnippet = 'For labels, we can easily render the HTML ourselves, saving all the API requests';
const singleNote = {
