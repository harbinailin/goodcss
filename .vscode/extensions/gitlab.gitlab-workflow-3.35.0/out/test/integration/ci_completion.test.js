...
const { createAndOpenFile, closeAndDeleteFile, getRepositoryRoot, } = require('./test_infrastructure/helpers');
...
describe('CI variable completion', () => {
    describe('.gitlab-ci.yml', () => {
        ...
        const openCompletion = async (position) => {
            return vscode.commands.executeCommand('vscode.executeCompletionItemProvider', gitlabCiYml, position);
        };
        beforeEach(async () => {
        ...
        it("won't complete when no dollar is found", async () => {
            ...
            assert.deepStrictEqual(completions.items.filter(item => item.kind === vscode.CompletionItemKind.Constant), []);
        });
        ...
        it('will handle multiple $ characters on the same line', async () => {
            ...
            assert.deepStrictEqual(start.character, text.length - 8, 'completion item should start at the position of the last $');
        });
    });
});
//# sourceMappingURL=ci_completion.test.js.map